...
def _as_numpy(tensors):
    """The zTensor writer takes numpy arrays; torch tensors are converted
    without copying where torch allows it."""
    out = {}
...
def benchmark_write(format_name, tensors, filepath):
    ...
    if format_name in ("ztensor", "ztensor_zerocopy"):
        ...
        ztensor.numpy.save_file(_as_numpy(tensors), filepath)

    elif format_name == "ztensor_zstd" or format_name.startswith("ztensor_zstd"):
        import ztensor.numpy

        # v2 exposes the zstd-seekable profile as on/off; the level is a
        # writer-internal choice, so the zstdN variants all measure the
        # same encoder.
        ztensor.numpy.save_file(_as_numpy(tensors), filepath, compression=True)
...
def benchmark_read(format_name, filepath, fastest=False):
    ...
    if format_name == "ztensor_zerocopy":
        ...
        loaded_tensors = ztensor.numpy.load_file(filepath, copy=not fastest)
...
def benchmark_read_selective(format_name, filepath, fraction=0.1):
    ...
    if format_name.startswith("ztensor") or format_name.startswith("zt_"):
        ...
        src = ztensor.open(filepath)
        all_keys = src.keys()
        step = max(1, int(1.0 / fraction))
        loaded_tensors = {k: src.read(k) for k in all_keys[::step]}
