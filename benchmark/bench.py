...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Data size in MB for single run (default: 512)",
    )
    parser.add_argument(
        "--runs",
        type=int,
    ...
    parser.add_argument(
        "--csv",
        type=str,
        default="bench_out/sweep_results.csv",
        help="CSV path for plot mode",
    )
    parser.add_argument(
        "--backend",
        type=str,
    ...
    parser.add_argument(
        "--data-style",
        type=str,
        choices=["random", "structured", "mixed_dtype"],
        default="random",
        help="Data generation style (default: random)",
    )
    parser.add_argument(
        "--scenario",
        type=str,
        choices=["full_load", "zero_copy", "selective", "fastest"],
        default="full_load",
        help="Benchmark scenario (default: full_load)",
    )
    parser.add_argument(
        "--dist",
        type=str,
        choices=["mixed", "small", "large", "llama-1b"],
        default="mixed",
        help="Tensor distribution (default: mixed)",
    )
    ...
    print(
        f"{'Format':<20} | {'Write (GB/s)':>12} | {'Read (s)':>12} | {'Read (GB/s)':>12} | {'File (MB)':>10}"
    )
    print("-" * 80)
    ...
    for fmt in formats:
        ...
        print(
            f"{fmt:<20} | {w_speed:>12.2f} | {r_lat:>12.4f} | {r_tp:>12.2f} | {file_mb:>10.1f}"
        )
